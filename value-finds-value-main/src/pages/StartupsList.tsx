...
import { foundersData, FounderStartup } from '../data/foundersData';
...
const uniqueEcosystems = Array.from(new Set(foundersData.map(s => s.city)));
...
const StartupsList = () => {
  ...
  const filteredStartups = foundersData.filter(s =>
    (ecosystem === '' || s.city === ecosystem) &&
    (searchTerm === '' || s.name.toLowerCase().includes(searchTerm.toLowerCase()))
  );

  const visibleStartups = isLoggedIn ? filteredStartups : foundersData.slice(0, 4);
              ...
              <div className="flex-1 flex flex-col md:flex-row md:items-center gap-2 md:gap-6 py-4 px-2 md:px-0">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-2 text-sm text-gray-600 mb-1">
                    <span className="flex items-center gap-1"><MapPin className="h-4 w-4 text-brand-yellow" />{startup.city}</span>
                    <span className="flex items-center gap-1"><Calendar className="h-4 w-4 text-brand-yellow" />{startup.employees.match(/\d{4}/) ? startup.employees.match(/\d{4}/)[0] : 'N/A'}</span>
                  </div>
